...
// Desactivar mostrar errores en la salida para evitar que se mezclen con JSON
ini_set("display_errors", 0);
ini_set("log_errors", 1);
ini_set("error_log", DIR."/php_error_log");

// Capturar cualquier salida de errores
ob_start();
...
class FilesController{
	...
	public function ajaxUploadFiles(){
		...
		if($folder->status == 200){
			...
			if($this->folder == 1){
				...
				if(move_uploaded_file($this->file["tmp_name"], $path)){
					...
					if($uploadData->status == 200){
						...
						$response = array(
							"status" => 400,
							"error" => "Error al guardar el archivo en la base de datos: ".($uploadData->results ?? "Error desconocido")
						);
					...
					$response = array(
						"status" => 400,
						"error" => "Error al mover el archivo al servidor. Verifique los permisos del directorio."
					);
				...
				// Error: folder no es 1 (servidor local)
				$response = array(
					"status" => 400,
					"error" => "Error: Solo se permite subir archivos al servidor local (folder 1)"
				);
			...
			// Error al obtener información del folder
			$errorMsg = isset($folder->results) ? $folder->results : "Error desconocido";
			$response = array(
				"status" => 400,
				"error" => "Error al obtener información del folder: ".$errorMsg
			);
		...
		// Si llegamos aquí sin respuesta, devolver error genérico
		$response = array(
			"status" => 500,
			"error" => "Error inesperado al procesar la subida del archivo"
		);
