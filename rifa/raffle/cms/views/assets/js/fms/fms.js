		...
		$.ajax({
			...
			success: function(response){
				...
				try {
					...
					console.error("Error al parsear respuesta JSON:", response);
					fncToastr("error", "Error al procesar la respuesta del servidor");
					$(".columnName"+i).parent().removeClass("itemsUp");
					$(".progressList"+i).html('<div class="alert alert-danger">Error al subir archivo</div>');
					$(".progressGrid"+i).html('<div class="alert alert-danger">Error al subir archivo</div>');
					return;
				}
				...
				if(jsonResponse.status == 200){
					...
					fncMatPreloader("off");
					var errorMessage = jsonResponse.error || "Error desconocido al subir el archivo";
					fncToastr("error", errorMessage);
					
					// Limpiar estados de carga
					$(".columnName"+i).parent().removeClass("itemsUp");
					$(".progressList"+i).html('<div class="alert alert-danger">'+errorMessage+'</div>');
					$(".progressGrid"+i).html('<div class="alert alert-danger">'+errorMessage+'</div>');
