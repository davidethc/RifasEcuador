  ...
  return (
    <main className="min-h-screen pt-8 md:pt-12 pb-8" aria-label="Confirmación de compra">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Enviar números por WhatsApp - Solo cuando la compra está completada */}
        {isCompleted && order.numbers && order.numbers.length > 0 && (
          <div className="mb-8 flex justify-center">
            <a
              href={`https://wa.me/593960948984?text=${encodeURIComponent(
                ...
                `• Orden: ${order.id}\n` +
                `• Sorteo: ${order.raffles?.title || 'Sorteo'}\n` +
                `• Números: ${order.numbers.join(', ')}\n` +
                `• Total: ${formatPrice(order.total)}\n\n` +
                `Guardo este mensaje para tener mis números a mano.`
              )}`}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-3 px-6 py-4 rounded-xl font-semibold transition-all font-[var(--font-dm-sans)]"
              style={{
            ...
            >
              ...
              <span>Enviar mis números por WhatsApp</span>
            </a>
          </div>
        )}
