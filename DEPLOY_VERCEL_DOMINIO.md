...
## Solución #2: Preview de Vercel (*.vercel.app) en blanco

Si la URL de **preview** (ej. `tu-proyecto-xxx.vercel.app`) sale en blanco o con 401:

- **Opción 2.1 – Permitir *.vercel.app en el middleware**  
  En este proyecto el middleware **ya permite** `altokeec.com`, `admin.altokeec.com` y `*.vercel.app` (lista explícita en `middleware.ts`). No se bloquea por host, así que el preview puede servirse con normalidad.

- **Opción 2.2 – No devolver 401 en `/`**  
  La app **no devuelve 401** en la raíz: `/` es ruta pública y, en rutas privadas sin sesión, se **redirige a `/login`** (no 401). Si en el futuro añades comprobación de sesión en middleware para `/`, usa **redirect 302/307 a `/login`** en lugar de 401 para que el preview no quede en blanco.

Más detalle en **VERCEL_PAGINA_BLANCA.md** (sección “Preview de Vercel”).
