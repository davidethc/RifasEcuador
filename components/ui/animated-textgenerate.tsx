    ...
    return (
      <div className={cn("flex flex-wrap items-center gap-1", contentClassName)}>
        {splitWords.map((word, idx) => {
          ...
          if (isVisible) {
            // Verificar si esta palabra tiene colores por letra
            const wordKey = word.toLowerCase().replace(/[.,!?;:]/g, '');
            const hasLetterColors = letterColors && letterColors[wordKey] && letterColors[wordKey].length > 0;
            
            const wordElement = hasLetterColors ? (
              <motion.span
                key={`${word}-${idx}`}
                initial={{
                  opacity: 0,
              ...
              >
                {word.split('').map((letter, letterIdx) => {
                  // Detectar si la letra es un signo de puntuación
                  const isPunctuation = /[.,!?;:]/.test(letter);
                  
                  // Si es signo de puntuación, usar color por defecto, sino usar el color de la letra
                  let letterColor = "text-black dark:text-white";
                  if (!isPunctuation) {
                    const cleanLetterIdx = word.substring(0, letterIdx).replace(/[.,!?;:]/g, '').length;
                    if (cleanLetterIdx < letterColors[wordKey].length) {
                      letterColor = letterColors[wordKey][cleanLetterIdx];
                    } else {
                      letterColor = letterColors[wordKey][letterColors[wordKey].length - 1] || "text-black dark:text-white";
                    }
                  }
                  
                  return (
                    <span
                      key={`${word}-${idx}-${letterIdx}`}
